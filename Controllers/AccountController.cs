...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    MySaniSoftContext ctx = new MySaniSoftContext();
                    JournalConnexion journal = new JournalConnexion() { Date = DateTime.Now, Id = Guid.NewGuid(), User = model.Email};
                    ctx.JournalConnexions.Add(journal);
                    ctx.SaveChanges();
        ...
        [Authorize]
        public ActionResult ResetPassword(string code="yandi")
        {
            return code == "yandi" ? View() : View("Error");
        }
