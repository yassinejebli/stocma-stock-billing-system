...
const SettingsProvider = ({ children }) => {
  ...
  const [articlesStatisticsModule, setArticlesStatisticsModule] =
    React.useState(null);
  const [clientLoyaltyModule, setClientLoyaltyModule] = React.useState(null);
  const [articlesNotSellingModule, setArticlesNotSellingModule] =
    React.useState(null);
  ...
  //FA client
  React.useEffect(() => {
    if (facturePayment) updateData(TABLE, facturePayment, facturePayment?.Id);
  }, [facturePayment]);

  React.useEffect(() => {
    if (factureCheque) updateData(TABLE, factureCheque, factureCheque?.Id);
  }, [factureCheque]);

  //Barcode
  React.useEffect(() => {
    if (barcode) updateData(TABLE, barcode, barcode?.Id);
  }, [barcode]);

  //BL
  React.useEffect(() => {
    if (BLPayment) updateData(TABLE, BLPayment, BLPayment?.Id);
  }, [BLPayment]);
  // React.useEffect(() => {
  //     if (BLDiscount)
  ...
  React.useEffect(() => {
    if (devisValidity) updateData(TABLE, devisValidity, devisValidity?.Id);
  }, [devisValidity]);

  React.useEffect(() => {
    if (devisPayment) updateData(TABLE, devisPayment, devisPayment?.Id);
  }, [devisPayment]);

  React.useEffect(() => {
    if (devisTransport) updateData(TABLE, devisTransport, devisTransport?.Id);
  }, [devisTransport]);
    ...
    getAllData(TABLE)
      .then((res) => {
        ...
        setArticleMarginModule(
          res.find((x) => x.Code === "module_article_margin")
        );
        setClientMarginModule(
          res.find((x) => x.Code === "module_client_margin")
        );
        ...
        setUtilisateursModule(
          res.find((x) => x.Code === "module_utilisateurs")
        );
        setRapportVenteModule(
          res.find((x) => x.Code === "module_rapport_vente")
        );
        setMouvementModule(res.find((x) => x.Code === "module_mouvement"));
        setPaiementModule(res.find((x) => x.Code === "module_paiement"));
        setArticleImageModule(
          res.find((x) => x.Code === "module_image_article")
        );
        setRestoreBLModule(res.find((x) => x.Code === "module_restoration_bl"));
        setArticlesStatisticsModule(
          res.find((x) => x.Code === "module_statistique_articles")
        );
        setClientLoyaltyModule(
          res.find((x) => x.Code === "module_client_fidelite")
        );
        setArticlesNotSellingModule(
          res.find((x) => x.Code === "module_article_non_vendus")
        );
        //Devis
