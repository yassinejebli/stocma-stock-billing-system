  ...
  } = useTable(
    {
      ...
      deleteRow: async (params) => {
        if (serverPagination) {
          ...
          if (deleteRow && ok) deleteRow(params);
        } else {
          deleteRow(params);
        }
      },
      ...
      >
        <Box ml={0.5}>
          <span style={{ fontSize: 11 }}>
            Nombre d'éléments : {serverPagination ? totalItems : rows.length}
          </span>
        </Box>
        {serverPagination && (
...
const EditableCell = ({
  ...
  const onBlur = () => {
    if (updateMyData) updateMyData(index, id, value);
  };
