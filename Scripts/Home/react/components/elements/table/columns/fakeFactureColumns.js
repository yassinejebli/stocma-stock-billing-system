        ...
        <>
          <FakeArticleAutocomplete
            ...
            onChange={(_, selectedValue) => {
              ...
              if (selectedValue) updateMyData(index, "Pu", selectedValue.PVD);
              if (
                data.filter((x) => !x.Article).length === 1 ||
                data.length === 1
              )
                addNewRow();

              const qteCell = document.querySelector(
                `#my-table #Qte-${index} input`
              );
              if (qteCell) {
                setTimeout(() => {
                  qteCell.focus();
          ...
          {value && (
            ...
            >
              ...
              >
                <span>{value.QteStock} en stock</span>
              </Tooltip>
            </Box>
          )}
        </>
        ...
        <>
          <Input
            ...
            onChange={({ target: { value: _value } }) =>
              updateMyData(index, id, _value)
            }
            onBlur={() => updateMyData(index, id, value)}
          />
          {console.log({ original })}
          {Boolean(value && original?.Article?.PA >= value) && (
            <Box mb={1}>
              <div
                style={{
                  marginLeft: "auto",
...
export const fakeFactureListColumns = (isEAS) => [
  {
    Header: "Client",
  ...
  },
  {
    ...
    type: inputTypes.text.description,
    accessor: (props) => {
      return props.DateEcheance
        ? format(new Date(props.DateEcheance), "dd/MM/yyyy")
        : "";
    },
    width: 50,
  },
  {
    ...
    width: 45,
    accessor: (props) => {
      const total = props.FakeFactureItems.reduce(
        (sum, curr) => (sum += curr.Pu * curr.Qte),
        0
      );
    ...
    Cell: ({ row: { original }, updateRow, deleteRow, print, convert }) => {
      return (
        <Box display="flex" justifyContent="flex-end">
          ...
          {isEAS && (
            <Tooltip title="Mhamid">
              <IconButton
                tabIndex={-1}
                size="small"
                onClick={() => convert(original)}
              >
                <PrintOutlinedIcon />
              </IconButton>
            </Tooltip>
