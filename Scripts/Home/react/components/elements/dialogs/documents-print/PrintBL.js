...
const PrintBL = ({ document, typePaiement, onClose, onExited, open }) => {
  ...
  const [showForm, setShowForm] = React.useState(
    typePaiement && !isClientDivers && canManagePaiementsClients
  );
  const [bigFormat, setBigFormat] = React.useState(
    useVAT || document?.WithDiscount
  );
        ...
        {document && (
          <>
            ...
            {showForm && !useVAT && !isClientDivers && (
              <Box mt={2}>
                <PaiementClientForm
                  amount={document[DOCUMENT_ITEMS]?.reduce(
                  ...
                  onSuccess={() => {
                    ...
                    const iframe =
                      window.document.getElementById("iframe-dialog");
