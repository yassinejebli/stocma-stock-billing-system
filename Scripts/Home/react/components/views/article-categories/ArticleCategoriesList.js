...
const ArticleCategoriesList = () => {
  ...
  const columns = React.useMemo(() => articleCategoriesColumns(), []);
  const [showArticleCategoriesModal, hideArticleCategoriesModal] = useModal(
    ({ in: open, onExited }) => (
      <SideDialogWrapper
        open={open}
        onExited={onExited}
        onClose={hideArticleCategoriesModal}
      >
