...
import { factureAchatListColumns } from "../../elements/table/columns/factureAchatColumns";
import PrintFacture from "../../elements/dialogs/documents-print/PrintFacture";
import AddIcon from "@material-ui/icons/Add";

const DOCUMENT = "FactureFs";
const EXPAND = [
  "Fournisseur($select=Id,Name)",
  "TypePaiement",
  "BonReceptions/BonReceptionItems/Article",
];
...
const FactureAchatList = () => {
  ...
  const columns = React.useMemo(() => factureAchatListColumns(), []);
  ...
  const updateRow = React.useCallback(async (id) => {
    history.push(`FactureAchat?FactureAchatId=${id}`);
  }, []);
    ...
    <>
      ...
      <Box mt={1} mb={2} display="flex" justifyContent="flex-end">
        <Button
          ...
          color="primary"
          startIcon={<AddIcon />}
          onClick={() => history.push("/FactureAchat")}
        >
          Nouvelle facture
        </Button>
      </Box>
