...
const Facture = () => {
  const { factureDiscount, setFactureDiscount, facturePayment, factureCheque } =
    useSettings();
  ...
  const FactureId = qs.parse(location.search, {
    ignoreQueryPrefix: true,
  }).FactureId;
  const isEditMode = Boolean(FactureId);
  const data = [].concat(
    ...selectedBonLivraisons.map((x) =>
      x.BonLivraisonItems.map((y) => ({
        Article: y.Article,
        Qte: y.Qte,
        Pu: y.Pu,
        Description: x.NumBon,
        Discount: y.Discount
          ? y.Discount + (y.PercentageDiscount ? "%" : "")
          : "",
      }))
    )
  );
  const discount = data.reduce((sum, curr) => {
    ...
    if (curr.Discount) {
      if (!isNaN(curr.Discount)) sum += Number(curr.Discount);
  ...
  const total = data.reduce((sum, curr) => (sum += curr.Pu * curr.Qte), 0);
  ...
  React.useEffect(() => {
    ...
    if (isEditMode) {
      setLoading(true);
      getSingleData(DOCUMENT, FactureId, [
        DOCUMENT_OWNER,
        "TypePaiement",
        "BonLivraisons/BonLivraisonItems/Article",
      ])
        .then((response) => {
  ...
  const save = async () => {
    ...
    const response = isEditMode
      ? await (await updateData(DOCUMENT, preparedData, Id, expand)).json()
    ...
    <>
      ...
      <Paper>
          ...
          <BonLivraisonAutocomplete
            ...
            onChange={(_, value) => {
              if (
                ...
                value?.find((x) => x.WithDiscount)
              )
                setFactureDiscount((_setting) => ({
                  ..._setting,
                  Enabled: true,
                }));
        ...
        <Box mt={2} display="flex" flexWrap="wrap">
          ...
          {factureCheque?.Enabled && paymentType?.IsBankRelated && (
            <Box width={240}>
              <TextField
                value={chequeNumber}
        ...
        <Box mt={4}>
          <Table
            ...
            addNewRow={addNewRow}
          />
          {errors.table && <Error>{errors.table}</Error>}
