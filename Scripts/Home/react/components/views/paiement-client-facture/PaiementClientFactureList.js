...
const EXPAND = ['TypePaiement', 'Client($select=Id,Name)', 'Facture/Client($select=Id,Name)', 'BonLivraisons/BonLivraisonItems'];
...
const PaiementClientFactureList = () => {
    ...
    const [showPaiementModal, hidePaiementModal] = useModal(({ in: open, onExited }) => (
        <SideDialogWrapper open={open} onExited={onExited} onClose={hidePaiementModal}>
            {selectedRow && <PaiementFactureClientForm
    ...
    return (
        <>
            <Box mt={1} mb={2} display="flex" justifyContent="space-between">
                {client && dateFrom && dateTo && <Box display="flex"><Button
                    variant="contained"
                    color="primary"
                    startIcon={<PrintIcon />}
