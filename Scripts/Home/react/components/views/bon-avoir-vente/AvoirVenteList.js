...
import { bonAvoirVenteListColumns } from '../../elements/table/columns/bonAvoirVenteColumns'
import AddIcon from '@material-ui/icons/Add';
import { useSite } from '../../providers/SiteProvider'

const DOCUMENT = 'BonAvoirCs'
const EXPAND = ['Client($select=Id,Name)', 'BonAvoirCItems']

const BonAvoirVenteList = () => {
    ...
    const updateRow = React.useCallback(async (id) => {
        history.push(`bon-avoir-vente?BonAvoirVenteId=${id}`);
    }, []);
        ...
        <>
            ...
            <Box mt={1} mb={2} display="flex" justifyContent="flex-end">
                <Button
                    ...
                    color="primary"
                    startIcon={<AddIcon />}
                    onClick={() => history.push('/bon-avoir-vente')}
                >
                    Nouveau bon d'avoir (vente)
                </Button>
            </Box>
            <Paper>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <TitleIcon noBorder title="Liste des bons d'avoir (vente)" Icon={DescriptionOutlinedIcon} />
                    <TextField
                        value={searchText}
...
export default BonAvoirVenteList;
