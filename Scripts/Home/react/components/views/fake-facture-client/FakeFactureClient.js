...
const FakeFacture = () => {
  const { facturePayment, factureCheque } = useSettings();
  ...
  const FakeFactureId = qs.parse(location.search, {
    ignoreQueryPrefix: true,
  }).FactureId;
  const isEditMode = Boolean(FakeFactureId);

  const columns = React.useMemo(() => fakeFactureColumns(), []);
  ...
  React.useEffect(() => {
    ...
    if (isEditMode) {
      setLoading(true);
      getSingleData(DOCUMENT, FakeFactureId, [
        DOCUMENT_OWNER,
        "TypePaiement",
        "FakeFactureItems/ArticleFacture",
      ])
        .then((response) => {
          ...
          setData(
            response.FakeFactureItems?.map((x) => ({
