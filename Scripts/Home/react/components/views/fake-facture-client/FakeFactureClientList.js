...
import PrintFakeFacture, {
  PrintNewFakeFacture,
} from "../../elements/dialogs/documents-print/PrintFakeFacture";
...
const FakeFactureClientList = () => {
  ...
  const columns = React.useMemo(() => fakeFactureListColumns(isEAS), [isEAS]);
  ...
  const [showModalNew, hideModalNew] = useModal(
    ({ in: open, onExited }) => {
      return (
        <PrintNewFakeFacture
          onExited={onExited}
  ...
  const refetchData = () => {
    ...
    getData(DOCUMENT, {}, filters, EXPAND)
      .then((response) => {
  ...
  const print2 = React.useCallback((document) => {
    setDocumentToPrint(document);
    showModalNew();
  }, []);
